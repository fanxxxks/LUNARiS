...
Requires fonttools[woff] (fonttools + brotli). The local build dependency path is
optional; normal Python package installations work too. Re-run after adding UI text.
"""
...
sys.path.insert(0, str(ROOT / '.codex-review' / 'font-build-deps'))
from fontTools import subset
from fontTools.ttLib import TTFont

directory = ROOT / 'vendor' / 'fonts'
texts = [ROOT / 'lunar-ui.html', ROOT / 'simulation-core.js', *ROOT.glob('lunar-*.js')]
...
report = {'sources': {}, 'textFiles': [p.name for p in texts], 'characters': len(codepoints)}

for original, family, output, required in [
    ('NotoSansSC', 'Lunaris Sans', 'lunaris-sans.woff2', codepoints),
    ('SpaceGrotesk', 'Lunaris Display', 'lunaris-display.woff2', set(range(32, 592)) | set(range(8192, 8304))),
]:
    source = directory / 'sources' / (original + '.ttf')
    font = TTFont(source)
    ...
    names = font['name']
    replacement = {1: family, 2: 'Regular', 3: family.replace(' ', '') + '-UI-Subset',
    ...
    font.close()
assert not report['sources']['NotoSansSC']['missingCjk'], 'UI contains unsupported Chinese text'
