...
(function (root) {
'use strict';
const config=Object.freeze({version:7,columns:5,rows:3,layers:5,pitchX:125,pitchZ:117,pitchY:92,width:115,height:78,depth:105,moduleScaleXZ:1.25,interiorWidth:102.5,interiorDepth:91.25,clearance:3,baseY:34,metresPerUnit:.1,roomCount:24,foldedScale:.02,retractShiftX:62.5,retractShiftZ:58.5,walkHeight:26});
const footprints=[
...
const portOffsets=Object.freeze({'+X':Object.freeze([config.width/2,config.walkHeight,0]),'-X':Object.freeze([-config.width/2,config.walkHeight,0]),'+Y':Object.freeze([24*config.moduleScaleXZ,76.5,-23*config.moduleScaleXZ]),'-Y':Object.freeze([24*config.moduleScaleXZ,.3,-23*config.moduleScaleXZ]),'+Z':Object.freeze([0,config.walkHeight,config.depth/2]),'-Z':Object.freeze([0,config.walkHeight,-config.depth/2])});
const foyerOffset=Object.freeze([0,config.walkHeight,0]),ladderOffset=Object.freeze([portOffsets['+Y'][0],config.walkHeight,portOffsets['+Y'][2]]);
...
function personnelNetwork(layout,options={}){
 ...
 for(const {room,node} of available){
  const p=slots[node],foyer=vertex(addPoint(p,foyerOffset),room,'foyer'),spine=vertex(addPoint(p,ladderOffset),room,'ladder');foyers[room]=foyer;roomNodes[room]=node;
  edge(foyer,spine,'internal',[vertices[foyer].point,addPoint(p,[ladderOffset[0],config.walkHeight,0]),vertices[spine].point]);
