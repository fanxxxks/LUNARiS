...
const phaseNames={ready:'已就绪',seal:'关闭舱门并隔离',disconnect:'断开服务接口',unlock:'释放停靠锁',depart:'离座与径向退出',cruise:'外围飞行与姿态控制',approach:'反推制动与接近',softCapture:'软捕获与对准',hardCapture:'机械硬锁定',leakTest:'接口检漏',equalize:'接口均压',connect:'恢复服务连接',open:'开启气密通道',dock:'停靠完成',done:'重构完成'};
...
const blocked=()=>$('block').checked?[C.bay(4,2)]:[];
const snapshot=()=>{if(!snapshotCache||snapshotCache.base!==baseLayout||snapshotCache.phases!==phases||snapshotCache.time!==time)snapshotCache={base:baseLayout,phases,time,state:C.state(baseLayout,phases,time)};return snapshotCache.state;};
function requestRender(){needsRender=true;if(!rafPending&&!inFrame){rafPending=true;lastNow=performance.now();requestAnimationFrame(frame);}}
...
function chooseView(name){
 ...
 if(name==='closeup')goal={yaw:st.orientations[selectedRoom][1]+.3,pitch:.16,distance:mobile()?440:330,target:new T.Vector3(p[0],p[1]+35,p[2])};
...
function showPlan(){
 ...
 putText('planDistance',metres(moves.reduce((sum,m)=>sum+C.length(m.path),0)));const mission=C.missions[task];putText('missionPurpose',mission?.purpose||'选择房间和空闲泊位，经外围飞行完成搬运、落座与气密对接。');
...
function enterManual(){
 const st=snapshot();baseLayout=st.layout.slice();baseElevators=C.config.baseY;moves=[];phases=[];duration=0;time=0;playing=false;manualMode=true;pendingManual=false;task='manual';previewMove=null;$('movePreview').hidden=true;estop=false;ready=true;roaming=false;cutaway=false;if(viewName==='roam'||viewName==='mechanism')chooseView('overview');showPlan();controls();drawUI(snapshot());audit('manual-start',{layout:baseLayout.slice()});
}
$('manual').onclick=()=>{
...
function moveAxis(axis,sign){
 ...
 if(to===undefined){notify(axis===1?'该方向没有更高或更低的泊位。可直接点击任意空闲泊位规划跨层飞行。':'该方向没有可用泊位。可以直接点击空闲泊位规划飞行。',true);return;}moveSelected(to);
}
axisButtons.forEach(b=>b.onclick=()=>moveAxis(Number(b.dataset.axis),Number(b.dataset.sign)));
function selectBay(node){const st=snapshot(),room=st.layout[node];if(room===null){if(manualMode)moveSelected(node);else notify('这是空闲停靠位。开启手动调度后点击这里，可预览飞行路线。');}else selectRoom(room);}
...
$('block').onchange=()=>{if(manualMode){playing=false;pendingManual=false;const st=snapshot();if(st.phase&&st.phase.nodePath.includes(C.bay(4,2))&&time<duration){$('block').checked=false;notify('当前计划使用 L5 · P03 泊位，请停靠后再封闭。',true);}controls();drawUI(snapshot());}else{compile(task,false);if(ready)notify('通道状态已更新，计划已重新验证。');}};
...
$('export').onclick=()=>{
 const data={version:'5.0',display:'LUNARIS lunar surface propulsion and pressure docking concept',coordinateSystem:'X horizontal, Y vertical, Z depth',metresPerSceneUnit:C.config.metresPerUnit,config:C.config,initialLayout:baseLayout,initialElevators:baseElevators,slots:C.slots,nodes:C.nodes,shafts:C.shafts,mission:C.missions[task]||null,moves,phases,predictedSeconds:duration,events,current:snapshot(),time,blocked:blocked(),scope:'geometry and sequence; not an engineering validation'};
...
function drawUI(st){
 ...
 const arriving=st.active===selectedRoom&&['softCapture','hardCapture','leakTest','equalize','connect','open','dock'].includes(st.type);
 const location=st.active===selectedRoom&&['depart','cruise','approach'].includes(st.type)?'运输中':C.nodes[arriving?st.phase.to:node]?.label||'运输中';
 ...
 putText('phase',estop?'已急停':cutaway?'对接结构 · 演示已暂停':!ready?'当前布局暂无有效路径':manualMode&&!phases.length?'手动调度 · 请选择方向':`${taskNames[task]} · ${time===0?'已就绪':phaseNames[st.type]}`);
 putText('time',`${fmt(time)} / ${fmt(duration)}`);$('timeline').value=String(time);putText('completed',String(st.finished));
 putText('detail',cutaway?'隐藏舱体，观察中央气密核心、连接通道与独立落座支承。':estop?'建筑保持当前状态，解除急停后继续。':pendingManual?'正在完成当前搬运，停靠后进入手动调度。':time===0?'推进器将舱段送往外围飞行空域，落座后锁定并恢复气密连接。':st.phase?`${st.active>=0?buildingNames[st.active]:'对接机构'} · ${C.nodes[st.phase.from].label} → ${C.nodes[st.phase.to].label} · ${phaseNames[st.type]}${st.type==='cruise'?' · Y = '+metres(st.head[1])+' m':''}`:st.type==='done'?'任务完成。可切换楼层观察布局，或开启手动六向调度。':'推进器将舱段送往外围飞行空域，落座后锁定并恢复气密连接。');
 $('flightTelemetry').hidden=st.active<0||time===0;putText('flightStage',phaseNames[st.type]||st.type);putText('flightSpeed',(Math.hypot(...(st.velocity||[0,0,0]))*C.config.metresPerUnit).toFixed(1));putText('flightThrust',(st.thrustKN||0).toFixed(1));putText('dockStatus',st.connected?'气密通道已连通':st.doorsClosed?`舱门隔离 · 接口压力 ${Math.round((st.pressure||0)*100)}%`:'准备关闭舱门');
 ...
 for(const b of mapButtons){const index=Number(b.dataset.node),room=st.layout[index],movingHere=Boolean(st.phase&&st.phase.nodePath.includes(index)&&Math.hypot(...C.slots[index].map((v,k)=>v-st.head[k]))<45),key=`${room}|${selectedRoom}|${movingHere}`;if(mapStates[index]===key)continue;mapStates[index]=key;b.classList.toggle('empty',room===null);b.classList.toggle('chosen',room===selectedRoom);b.classList.toggle('active',movingHere);b.setAttribute('aria-label',`${C.nodes[index].label}：${room===null?'空闲':buildingNames[room]}`);b.title=`${C.nodes[index].label} · ${room===null?'空闲泊位':buildingNames[room]}`;}
 const busy=phases.length>0&&time<duration;
 axisButtons.forEach(b=>{const to=C.neighbor(node,Number(b.dataset.axis),Number(b.dataset.sign)),available=to!==undefined&&st.layout[to]===null&&!blocked().includes(to)&&!blocked().includes(node);b.disabled=Boolean(estop||busy||!available);});
 if(manualMode)putText('manualHint',estop?'已急停，请先解除急停。':busy?'建筑运输中，停靠后可继续调度。':previewMove?'已生成预览。确认飞行后执行完整离座与对接流程。':'点击空闲泊位预览飞行；六向按钮按方位选择泊位。' );
}
...
function draw(dt,now){
 const st=snapshot();updateInstances(st.positions,visibleFloor,cutaway,st.orientations,sectionMode?selectedRoom:-1);updateFlightVisual(st,time);
 ...
 const unavailable=st.active>=0&&!st.connected?st.active:-1,walkLayout=st.layout.slice();if(st.active>=0&&st.connected&&['connect','open','dock'].includes(st.type)){walkLayout[walkLayout.indexOf(st.active)]=null;walkLayout[st.phase.to]=st.active;}
 const walkingKey=walking?walkLayout.join(',')+'|'+unavailable+'|'+visibleFloor:'off';
 if(walking&&walkingKey!==walkLineKey){const ids=(C.missions[task]?.roomIds||[1,2,7]).filter(id=>id!==unavailable);drawWalkPath(C.walkMetrics(walkLayout,ids).routes,walkingKey);}else if(!walking)drawWalkPath([],walkingKey);walkGroup.visible=walking&&!cutaway;
 ...
 activeLabel.hidden=!$('labels').checked||!r.userData.visible||v.z>1||v.z< -1||sx<60||sx>w-60||sy<70||sy>h-20;const labelText=`${buildingNames[labelRoom]}${st.type==='cruise'?'  ↑↓ Y '+metres(st.head[1])+' m':''}`;if(activeLabel.textContent!==labelText)activeLabel.textContent=labelText;if(!activeLabel.hidden){activeLabel.style.transform=`translate3d(${sx}px,${sy}px,0) translate(-50%,-100%)`;activeLabel.style.left='0';activeLabel.style.top='0';}
...
function frame(now){
 rafPending=false;inFrame=true;const realDt=Math.max(.0001,(now-lastNow)/1000),dt=Math.min(.1,realDt);lastNow=now;
 ...
 inFrame=false;if(!rafPending&&(playing||cameraChanging()||(roaming&&held.size>0)||needsRender)){rafPending=true;requestAnimationFrame(frame);}
}
requestRender();
})();
