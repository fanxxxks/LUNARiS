const taskNames={vertical:'跨层换位',cascade:'三层重组',district:'街区平移',manual:'手动调度'};
...
let baseLayout=C.initial.slice(),baseElevatorY=24,moves=[],phases=[],duration=0,time=0,playing=false,estop=false,task='vertical',ready=false,events=[];
let selectedRoom=7,visibleFloor=-1,manualMode=false,pendingManual=false,cutaway=false,spread=0,viewName='overview',roaming=false,routeKey='',toastTimer;
let yaw=.7,pitch=.34,distance=950,target=new T.Vector3(24,137,0),goal={yaw:.7,pitch:.34,distance:950,target:new T.Vector3(24,137,0)};
const freePosition=new T.Vector3(),held=new Set();let freeYaw=0,freePitch=0;
const mobile=()=>window.innerWidth<=760;
const overviewDistance=()=>mobile()?1090:950;
...
const blocked=()=>$('block').checked?[C.lift(1)]:[];
const snapshot=()=>C.state(baseLayout,phases,time,baseElevatorY);
...
function chooseView(name){
 ...
 if(name==='closeup')goal={yaw:.7,pitch:.32,distance:mobile()?440:370,target:new T.Vector3(p[0],p[1]+35,p[2])};
 else if(name==='topview')goal={yaw:0,pitch:1.53,distance:mobile()?1130:1010,target:new T.Vector3(32,visibleFloor<0?110:24+visibleFloor*96,0)};
 else if(name==='elevation')goal={yaw:.15,pitch:.055,distance:mobile()?1160:920,target:new T.Vector3(28,160,0)};
 else goal={yaw:.7,pitch:cutaway?.39:.34,distance:cutaway?overviewDistance()+180:overviewDistance(),target:new T.Vector3(24,cutaway?166:137,0)};
 }
 updateViewButtons();
}
...
function compile(name,auto=true){
 task=name;manualMode=false;pendingManual=false;estop=false;playing=false;time=0;baseLayout=C.initial.slice();baseElevatorY=24;events=[];selectedRoom=name==='district'?7:19;
...
function enterManual(){
 const st=snapshot();baseLayout=st.layout.slice();baseElevatorY=st.elevatorY;moves=[];phases=[];duration=0;time=0;playing=false;manualMode=true;pendingManual=false;task='manual';estop=false;ready=true;roaming=false;cutaway=false;if(viewName==='roam'||viewName==='mechanism')chooseView('overview');showPlan();controls();drawUI(snapshot());audit('manual-start',{layout:baseLayout.slice()});
}
$('manual').onclick=()=>{
 if(manualMode){manualMode=false;controls();return;}
 if(estop){notify('请先解除急停，再开始手动调度。',true);return;}
 const st=snapshot();if(time>0&&time<duration&&st.active>=0){pendingManual=true;playing=true;cutaway=false;notify('当前建筑停靠后进入手动调度，保留已完成的布局。');controls();}else enterManual();
};
function moveSelected(to){
 ...
 if(from===to)return;
 try{const m=C.move(layout,from,to,blocked());baseLayout=layout.slice();baseElevatorY=current.elevatorY;moves=[m];const tl=C.timeline(moves,32,baseElevatorY);phases=tl.phases;duration=tl.duration;time=0;playing=true;ready=true;showPlan();audit('manual-move',{room:selectedRoom,from,to,path:m.path});controls();drawUI(snapshot());}
...
function moveAxis(axis,sign){
 ...
 if(to===undefined){notify(axis===1?'建筑须先平移至右侧升降塔，才能沿 Y 轴换层。':'该方向已到达轨道边界。',true);return;}moveSelected(to);
}
axisButtons.forEach(b=>b.onclick=()=>moveAxis(Number(b.dataset.axis),Number(b.dataset.sign)));
function selectBay(node){const st=snapshot(),room=st.layout[node];if(room===null){if(manualMode)moveSelected(node);else notify('这是空闲升降位。开启手动调度后可将选中建筑送到这里。');}else selectRoom(room);}
...
$('block').onchange=()=>{if(manualMode){playing=false;pendingManual=false;const st=snapshot();if(st.phase&&st.phase.nodePath.includes(C.lift(1))&&time<duration){$('block').checked=false;notify('当前搬运经过 L2 升降位，请停靠后再封闭。',true);}controls();drawUI(snapshot());}else{compile(task,false);if(ready)notify('通道状态已更新，计划已重新验证。');}};
...
$('quality').onchange=()=>{highQuality=$('quality').value==='high';renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,highQuality?1.6:1));renderer.shadowMap.enabled=true;sun.shadow.mapSize.set(highQuality?2048:1024,highQuality?2048:1024);if(sun.shadow.map){sun.shadow.map.dispose();sun.shadow.map=null;}$('qualityLabel').textContent=highQuality?'精细画质':'均衡画质';resize();};
...
$('lighting').onchange=()=>{
 const mode=$('lighting').value;sun.color.set(mode==='day'?'#f4f4ea':mode==='night'?'#8cabc9':'#ffead1');sun.intensity=mode==='night'?.22:mode==='day'?4.1:3.7;ambient.intensity=mode==='night'?.62:mode==='day'?2:1.7;rim.intensity=mode==='night'?.65:1.25;mats.warm.emissiveIntensity=mode==='night'?3.3:1.1;mats.cool.emissiveIntensity=mode==='night'?2.8:.55;stars.material.opacity=mode==='night'?.72:.42;postMat.uniforms.bloom.value=mode==='night'?.22:.1;
};
$('export').onclick=()=>{
 const data={version:'3.0',display:'LUNARIS multi-level architectural concept',coordinateSystem:'X horizontal, Y vertical, Z depth',metresPerSceneUnit:C.config.metresPerUnit,config:C.config,initialLayout:baseLayout,initialElevatorY:baseElevatorY,slots:C.slots,nodes:C.nodes,moves,phases,predictedSeconds:duration,events,current:snapshot(),time,blocked:blocked(),scope:'geometry and sequence; not an engineering validation'};
...
function drawUI(st){
 ...
 $('phase').textContent=estop?'已急停':cutaway?'结构分解 · 搬运已暂停':!ready?'升降通道封闭 · 暂无有效路径':manualMode&&!phases.length?'手动调度 · 请选择方向':`${taskNames[task]} · ${time===0?'已就绪':phaseNames[st.type]}`;
 $('time').textContent=`${fmt(time)} / ${fmt(duration)}`;$('timeline').value=String(time);$('completed').textContent=String(st.finished);
 $('detail').textContent=cutaway?'楼层分开展示承重骨架与运输轨道；实际调度坐标保持不变。':estop?'建筑保持当前状态，解除急停后继续。':pendingManual?'正在完成当前搬运，停靠后进入手动调度。':time===0?'建筑紧密排列，跨层运输通过右侧升降塔进行。':st.phase?`${st.active>=0?buildingNames[st.active]:'共享升降机构'} · ${C.nodes[st.phase.from].label} → ${C.nodes[st.phase.to].label} · ${phaseNames[st.type]}${st.type==='elevate'?' · Y = '+metres(st.head[1])+' m':''}`:st.type==='done'?'任务完成。可切换楼层观察布局，或开启手动六向调度。':'建筑紧密排列，跨层运输通过右侧升降塔进行。';
 ...
 if(manualMode)$('manualHint').textContent=estop?'已急停，请先解除急停。':busy?'建筑运输中，停靠后可继续调度。':C.nodes[node]?.lift?'已进入升降塔，可使用 Q / E 或 ±Y 跨层。':'WASD 平移 · Q / E 升降。灰色方向表示占用或轨道边界。';
}
...
function draw(dt,now){
 const st=snapshot();spread+=(Number(cutaway)-spread)*Math.min(1,dt*6);updateInstances(st.positions,visibleFloor,spread);
 ...
 updateRoute(st);
 if((viewName==='closeup'||$('follow').checked)&&playing&&st.active>=0&&!roaming){goal.target.set(st.head[0],st.head[1]+35,st.head[2]);}
 ...
 camera.updateMatrixWorld();const labelRoom=st.active>=0?st.active:selectedRoom,r=rooms[labelRoom],v=r.position.clone().add(new T.Vector3(0,85,0)).project(camera),w=stage.clientWidth,h=stage.clientHeight,sx=(v.x+1)*w/2,sy=(1-v.y)*h/2;
...
function resize(){const w=Math.max(1,stage.clientWidth),h=Math.max(1,stage.clientHeight);renderer.setSize(w,h);camera.aspect=w/h;camera.updateProjectionMatrix();const size=renderer.getDrawingBufferSize(new T.Vector2());targetRT.setSize(size.x,size.y);postMat.uniforms.resolution.value.copy(size);}
