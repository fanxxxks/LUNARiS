...
function connectivityCache(){
 ...
 return connectivityCache.value={group,owned,leaf:batch('recessed-hatch-leaves',mats.pale,288),inset:batch('hatch-inset-panels',mats.glassDark,288),handle:batch('hatch-lock-bars',mats.gold,288),shell:batch('telescoping-link-walls',mats.dark,1600),ribs:batch('connector-frames-and-ladder',mats.silver,4000),glass:batch('connector-side-glazing',mats.glass,400),matrix:new T.Object3D(),v:new T.Vector3(),q:new T.Quaternion(),doorKeys:[],graphKey:null,linkKey:null,links:[],effectiveLayout:C.initial.slice(),lastState:null,visible:true};
}
function connectionFaces(){return [{axis:0,sign:1,dir:'+X',p:[46,26,0],rot:[0,Math.PI/2,0]},{axis:0,sign:-1,dir:'-X',p:[-46,26,0],rot:[0,-Math.PI/2,0]},{axis:2,sign:1,dir:'+Z',p:[0,26,42],rot:[0,0,0]},{axis:2,sign:-1,dir:'-Z',p:[0,26,-42],rot:[0,Math.PI,0]},{axis:1,sign:1,dir:'+Y',p:[24,76.5,-23],rot:[-Math.PI/2,0,0]},{axis:1,sign:-1,dir:'-Y',p:[24,.3,-23],rot:[Math.PI/2,0,0]}];}
function connectionWrite(cache,batch,index,origin,quaternion,local,scale){
 const d=cache.matrix;d.position.fromArray(origin).add(cache.v.fromArray(local).applyQuaternion(quaternion));d.quaternion.copy(quaternion);d.scale.fromArray(scale);d.updateMatrix();batch.setMatrixAt(index,d.matrix);batch.instanceMatrix.addUpdateRange?.(index*16,16);
}
function updateConnectivity(st,options={}){
 ...
 for(let id=0;id<C.config.roomCount;id++){
  ...
  for(let f=0;f<faces.length;f++){
   const face=faces[f],q=s.q.setFromEuler(new T.Euler(...face.rot)),o=p.map((v,k)=>v+face.p[k]),h=face.axis===1?13.5:22.4,open=doorOpen[id][face.dir]||0;
   ...
   for(let side=0;side<2;side++){
    ...
    connectionWrite(s,s.leaf,index,o,q,[x,0,-2.15],[6.85*scale,h*scale,.65*scale]);
    connectionWrite(s,s.inset,index,o,q,[x,0,-1.78],[5.55*scale,(h-3.4)*scale,.12*scale]);
    connectionWrite(s,s.handle,index,o,q,[x-sign*2.25,-2,-1.58],[.5*scale,4*scale,.32*scale]);
   }
  }
 }
 ...
 if(linkKey!==s.linkKey){
  ...
  if(show)for(const e of s.links){
   ...
   if(p<=.001)continue;
   const levels=[C.nodes[e.nodeA]?.level,C.nodes[e.nodeB]?.level];if(floor>=0&&!levels.includes(floor))continue;
   const delta=new T.Vector3(...b).sub(new T.Vector3(...a)),len=delta.length();if(len<.01)continue;
   const q=new T.Quaternion().setFromUnitVectors(new T.Vector3(0,0,1),delta.normalize()),half=len*.5*p,h=axis===1?14:23;
   if(p>.995)count++;
   ...
   for(const side of [0,1]){
    ...
    for(const x of [-7.55,7.55])connectionWrite(s,s.shell,wi++,a,q,[x,0,center],[1.05,h+2,half]);
    for(const y of [-h/2-.5,h/2+.5])connectionWrite(s,s.shell,wi++,a,q,[0,y,center],[14.1,1,half]);
    if(axis!==1){for(const x of [-8.11,8.11])connectionWrite(s,s.glass,gi++,a,q,[x,2,center],[.15,h*.52,half*.75]);}
    for(const t of [0,.48,1]){
     const z=side?len-half*t:half*t;
     for(const x of [-8.15,8.15])connectionWrite(s,s.ribs,ri++,a,q,[x,0,z],[.65,h+3.1,.65]);
     for(const y of [-h/2-1.2,h/2+1.2])connectionWrite(s,s.ribs,ri++,a,q,[0,y,z],[16.8,.65,.65]);
    }
   }
   if(axis===1&&p>.995){
    for(const y of [-5,5])connectionWrite(s,s.ribs,ri++,a,q,[6,y,len/2],[.5,.5,len]);
    for(let z=1.5;z<len;z+=3.4)connectionWrite(s,s.ribs,ri++,a,q,[6,0,z],[.5,10,.5]);
   }
  }
  s.connections=count;
...
function updateTrafficVisual(data,options={}){
 ...
 const position=s.geometry.attributes.position.array,color=s.geometry.attributes.color.array,density=s.geometry.attributes.density.array;
 ...
 for(let id=0;id<C.config.roomCount;id++){
  ...
  function value(x,z){let field=load*.08;for(const e of roomLinks[id]){const endX=e.point[0]-p[0],endZ=e.point[2]-p[2],distance=pointSegmentDistance(x,z,0,0,endX,endZ);field+=e.flow/max*Math.exp(-distance*distance/175);}return Math.min(1,field);}
  ...
  const nx=8,nz=8;for(let x=0;x<nx;x++)for(let z=0;z<nz;z++){
   ...
   const world=(a,b)=>[p[0]+a,p[1]+14.22,p[2]+b];quad(world(x0,z0),world(x1,z0),world(x1,z1),world(x0,z1),[value(x0,z0),value(x1,z0),value(x1,z1),value(x0,z1)]);
  }
 }
 for(const e of links){
  ...
  const side=axis===0?[0,0,6.6]:[6.6,0,0],add=(p,k)=>p.map((v,i)=>v+side[i]*k);
