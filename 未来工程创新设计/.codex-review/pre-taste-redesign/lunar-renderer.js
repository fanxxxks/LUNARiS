...
const aoRT=new T.WebGLRenderTarget(1,1,{type:T.HalfFloatType,depthBuffer:false});
...
vec3 bright(vec2 uv){vec3 c=texture2D(tScene,uv).rgb;float b=max(c.r,max(c.g,c.b)),k=clamp((b-1.15)/1.35,0.,1.);return c*k*k/(1.+b*.28);}
...
vec3 displayColor(vec2 uv){vec3 x=texture2D(tScene,uv).rgb*exposure;x=clamp((x*(2.51*x+.03))/(x*(2.43*x+.59)+.14),0.,1.);return mix(x*12.92,1.055*pow(x,vec3(1./2.4))-.055,step(vec3(.0031308),x));}
...
float ambientOcclusion(){if(ao<=0.)return 1.;vec4 p=projectionInverse*vec4(vUv*2.-1.,texture2D(tDepth,vUv).r*2.-1.,1.);float z=-p.z/p.w;vec2 px=1./aoResolution;float sum=0.,weight=0.;for(int i=0;i<4;i++){vec2 o=vec2(float(i-2*(i/2))-.5,float(i/2)-.5);vec2 a=texture2D(tAO,vUv+o*px).rg;float w=1./(1.+abs(a.y-z)*2.);sum+=a.x*w;weight+=w;}return 1.-(1.-sum/max(weight,.0001))*ao;}
void main(){vec3 col=antialias()*ambientOcclusion();vec3 glow=texture2D(tBloom,vUv).rgb;col+=glow*bloom;vec2 q=vUv-.5;col*=1.-.13*dot(q,q);gl_FragColor=vec4(clamp(col,0.,1.),1.);}`});
...
function setLighting(mode){sun.color.set(mode==='day'?'#f5f7f6':mode==='night'?'#a0bcd1':'#f6f1e8');sun.intensity=mode==='night'?.34:mode==='day'?4.1:3.6;ambient.intensity=mode==='night'?.55:mode==='day'?1.25:.95;rim.intensity=mode==='night'?.85:1.65;mats.warm.emissiveIntensity=mode==='night'?2.6:1.45;mats.cool.emissiveIntensity=mode==='night'?2.2:1.15;mats.innerWall.emissiveIntensity=mode==='night'?.45:.08;mats.interior.emissiveIntensity=mode==='night'?.3:.08;stars.material.opacity=mode==='night'?.67:.34;workLights.forEach((l,i)=>l.intensity=[1400,1600,900,700][i]*(mode==='night'?1.3:.6));postMat.uniforms.bloom.value=mode==='night'?.2:.14;renderer.shadowMap.needsUpdate=true;resetAccumulation();}
