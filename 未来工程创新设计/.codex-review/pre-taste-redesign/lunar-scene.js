...
envPanel(1300,500,-500,650,350,'#f5f7f5',3.5);envPanel(700,600,650,380,-320,'#adc7d8',1.8);envPanel(1600,500,0,180,-1100,'#b7c9d5',.6);envPanel(1800,1800,0,-500,0,'#434a4f',.45);
...
const mats={
 frame:new T.MeshStandardMaterial({color:'#96a5ad',map:alloyMap,roughnessMap:roughMap,roughness:.6,metalness:.76,envMapIntensity:.85}),
 hull:new T.MeshStandardMaterial({color:'#edf1f0',map:alloyMap,roughnessMap:roughMap,roughness:.66,metalness:.2,envMapIntensity:.65}),
 silver:new T.MeshStandardMaterial({color:'#c0c8cd',roughnessMap:brushedMap,roughness:.46,metalness:.94,envMapIntensity:1.1}),
 pale:new T.MeshStandardMaterial({color:'#b5c1c2',roughness:.41,metalness:.6}),
 dark:new T.MeshStandardMaterial({color:'#202b32',roughness:.65,metalness:.72,envMapIntensity:.8}),
 ...
 glass:new T.MeshPhysicalMaterial({color:'#7a9cab',roughness:.13,metalness:.2,transparent:true,opacity:.29,depthWrite:false,forceSinglePass:true,clearcoat:1,clearcoatRoughness:.06,envMapIntensity:1.35,side:T.DoubleSide}),
...
for(let type=0;type<6;type++){const ids=roomTypes.flatMap((t,i)=>t===type?[i]:[]);if(!ids.length)continue;for(const part of moduleParts(type)){part.geometry.scale(moduleScaleXZ,1,moduleScaleXZ);part.geometry.computeBoundingSphere();const layer=part.userData.layer||'body',match=sharedParts.find(p=>p.part.material===part.material&&p.layer===layer&&sameGeometry(p.part.geometry,part.geometry));if(match){match.ids.push(...ids);part.geometry.dispose();}else sharedParts.push({part,ids:ids.slice(),layer});}}
...
const roomAtlas=texture(2048,512,(ctx,w,h)=>{const cw=w/6,ch=h/4;roomTypes.forEach((type,id)=>{const x=(id%6)*cw,y=Math.floor(id/6)*ch,palette=roomPalette[type];ctx.fillStyle='#101d28';ctx.fillRect(x,y,cw,ch);ctx.fillStyle=palette.color;ctx.fillRect(x+8,y+8,cw-16,10);ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillStyle='#f4f0e8';ctx.font='bold 74px Arial';ctx.fillText('R'+String(id+1).padStart(2,'0'),x+cw/2,y+63);ctx.fillStyle=palette.color;ctx.font='bold 24px Arial';ctx.fillText(palette.code,x+cw/2,y+108);});},null,true);
...
for(const sign of [-1,1]){identityFace(22,8,[0,60,sign*(roomDepth/2-.08)],[0,sign<0?Math.PI:0,0]);identityFace(22,8,[sign*(roomWidth/2-.08),60,0],[0,sign*Math.PI/2,0]);}
identityFace(25,8,[0,68.92,44],[-Math.PI/2,0,0]);
