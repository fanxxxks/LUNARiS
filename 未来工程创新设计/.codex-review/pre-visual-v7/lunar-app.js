...
let yaw=.64,pitch=.25,distance=1110,target=new T.Vector3(-24,212,0),goal={yaw:.64,pitch:.25,distance:1110,target:new T.Vector3(-24,212,0)};
const freePosition=new T.Vector3(),held=new Set();let freeYaw=0,freePitch=0;
const mobile=()=>window.innerWidth<=760;
const overviewDistance=()=>mobile()?1190:1110;
...
function setFloor(level){requestRender();applyFloorSelection(level);if(level<0)chooseView('overview');else{cutaway=false;roaming=false;viewName='layer';goal={yaw:.64,pitch:.72,distance:mobile()?960:800,target:new T.Vector3(level>=2?-45:-10,C.config.baseY+level*C.config.pitchY+25,0)};updateViewButtons();}drawUI(snapshot());}
...
function chooseView(name){
 ...
 if(name==='closeup'){playing=false;controls();applyFloorSelection(Math.max(0,Math.min(C.config.layers-1,Math.round((p[1]-C.config.baseY)/C.config.pitchY))));goal={yaw:.24,pitch:.4,distance:mobile()?330:270,target:new T.Vector3(p[0],p[1]+35,p[2])};}
 else if(name==='topview')goal={yaw:0,pitch:1.53,distance:mobile()?1200:1080,target:new T.Vector3(-25,visibleFloor<0?180:C.config.baseY+visibleFloor*C.config.pitchY,0)};
 else if(name==='elevation')goal={yaw:.15,pitch:.055,distance:mobile()?1180:1100,target:new T.Vector3(-24,212,0)};
 else goal={yaw:.64,pitch:cutaway?.3:.25,distance:overviewDistance(),target:new T.Vector3(-24,212,0)};
 }
 updateViewButtons();
}
...
$('walkthrough').onclick=()=>{walking=!walking;$('walkthrough').classList.toggle('active',walking);$('walkthrough').setAttribute('aria-pressed',String(walking));requestRender();if(walking)notify('绿色线显示科研舱之间经接缝通道与独立人员梯的通行示意；正在转运的舱段暂不可通行。');};
...
$('export').onclick=()=>{
 const data={version:'6.0',display:'LUNARIS compact stacked modules and integrated XYZ framework',coordinateSystem:'X horizontal, Y vertical, Z depth',metresPerSceneUnit:C.config.metresPerUnit,config:C.config,initialLayout:baseLayout,initialElevators:baseElevators,slots:C.slots,nodes:C.nodes,shafts:C.shafts,mission:C.missions[task]||null,moves,phases,predictedSeconds:duration,events,current:snapshot(),time,blocked:blocked(),scope:'geometry and sequence; not an engineering validation'};
...
function draw(dt,now){
 const st=snapshot();updateInstances(st.positions,visibleFloor,cutaway,st.orientations,sectionMode?selectedRoom:-1);updateMechanism(st,visibleFloor,cutaway);
 ...
 const unavailable=st.active>=0&&!st.connected?st.active:-1,walkLayout=st.layout.slice();if(st.active>=0&&st.connected&&['dock'].includes(st.type)){walkLayout[walkLayout.indexOf(st.active)]=null;walkLayout[st.phase.to]=st.active;}
