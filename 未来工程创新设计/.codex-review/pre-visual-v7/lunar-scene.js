...
function floorBox(parent,w,h,d,x,y,z,mat=mats.frame,record=false){
 let pieces=[{min:[x-w/2,y-h/2,z-d/2],max:[x+w/2,y+h/2,z+d/2]}];
 for(const pocket of carriagePockets){const next=[];for(const piece of pieces){
...
for(const shaft of C.shafts){
 ...
 box(g,roomWidth,3.2,roomDepth,0,-2.1,0,mats.gold);
 box(g,roomWidth-4,.6,roomDepth-4,0,-.6,0,mats.dark);
...
function moduleParts(type){
 ...
 // Compact rear service panel and protected handling interfaces stay inside the transport envelope.
 for(const x of [-39,39]){box(g,4,16,6,x,23,-32,mats.dark);box(g,2.4,12,.7,x,23,-35.4,mats.silver);}
 roundedBox(g,20,24,1.2,3,0,26,-39.3,mats.silver);box(g,13,17,.8,0,26,-40.3,mats.dark);box(g,.5,14,.6,0,26,-40.8,mats.cool);
