...
assert.equal(C.config.pitchY-height,14);assert.equal(C.config.metresPerUnit,.1);assert.equal(C.config.version,6);
...
let walkSegments=0;
for(const layout of [C.initial,C.preset('vertical').target,C.preset('cascade').target]){
 const metric=C.walkMetrics(layout);assert.ok(metric.schematic);assert.equal(metric.routes.length,3);
 for(const route of metric.routes){assert.ok(route.connected);assert.ok(route.distance>0);assert.ok(Math.abs(route.distance-route.walkingDistance-route.verticalDistance)<1e-6);
  for(let i=1;i<route.points.length;i++){
   const a=route.points[i-1],b=route.points[i],axis=a.findIndex((v,k)=>Math.abs(v-b[k])>1e-6);if(axis<0)continue;
   assert.equal(a.filter((v,k)=>Math.abs(v-b[k])>1e-6).length,1,'personnel seam route must be orthogonal');
   for(const [node,r] of layout.entries())if(r!==null){const room=bodyBounds(C.slots[node]),enters=room.min.every((v,k)=>k===axis?Math.max(a[k],b[k])>v+1e-6&&Math.min(a[k],b[k])<room.max[k]-1e-6:a[k]>v+1e-6&&a[k]<room.max[k]-1e-6);assert.ok(!enters,'personnel route crosses an occupied room instead of the seam');}
   walkSegments++;
  }
 }
}
...
console.log(`V6 compact clearances PASS: ${carriageChecks} folded/deploy carriage checks, ${walkSegments} orthogonal seam-route segments outside occupied rooms, cached and serialized seeking, zero room attitude and 0.5–16× terminal-state consistency.`);
