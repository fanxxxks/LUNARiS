 ...
 * Run from any directory: node .codex-review/v5-performance-test.cjs
 */
'use strict';
...
test('A moving room updates only that room in its material batches', () => {
  ...
  const phase = probeRead().phases.find(p => ['depart', 'cruise', 'approach'].includes(p.type));
...
test('Flight pose changes contain finite Euler YXZ orientations and leave parked room transforms unchanged', () => {
  element('reset').click(); tick();
  const phase = probeRead().phases.find(p => p.type === 'cruise');
  ...
  assert.ok(after.orientations[phase.room].some((v, i) => Math.abs(v - pose[i]) > .001), 'Cruise changes attitude');
