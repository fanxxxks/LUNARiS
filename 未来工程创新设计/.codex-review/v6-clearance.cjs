/* Read-only continuous geometry audit of the compact V6 frame. Actual Three
...
'use strict';
const assert=require('node:assert/strict'),fs=require('node:fs'),path=require('node:path'),{createHarness}=require('./v6-performance-test.cjs');
...
for(const o of scene.children.filter(o=>o.isMesh)){const candidate=faces(o,o.isInstancedMesh?'rocks':'terrain');for(const f of candidate)if(f.max[0]>-270&&f.min[0]<270&&f.max[2]>-165&&f.min[2]<165&&f.max[1]>20)staticFaces.push(f);}
...
assert.ok(moduleMin[0]>=-46.001&&moduleMax[0]<=46.001&&moduleMin[1]>=-.001&&moduleMax[1]<=78.001&&moduleMin[2]>=-42.001&&moduleMax[2]<=42.001,'actual room geometry exceeds planner');
