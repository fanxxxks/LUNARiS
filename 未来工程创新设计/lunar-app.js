...
const panelOpeners={viewPanel:'openViews',floorPanel:'openFloors'};
function showPanel(id,focus=false){
 const previous=activePanel;activePanel=id;held.clear();
...
document.addEventListener('click',event=>{if(activePanel&&!event.target.closest('.view-cluster,.inspection-stack,.top-actions')&&event.target.tagName!=='CANVAS')showPanel(null);});
...
function showPlan(){
 putText('missionIndex',({vertical:'01',cascade:'02',district:'03',manual:'M'})[task]);
...
function selectRoom(room){previewMove=null;$('movePreview').hidden=true;requestRender();selectedRoom=room;drawUI(snapshot());if(!document.body.classList.contains('reduced'))showPanel('floorPanel');if(viewName==='closeup')chooseView('closeup');}
...
document.addEventListener('keydown',e=>{
 if(e.key==='Escape'){held.clear();if(!$('settings').hidden)closeSettings();else if(activePanel)showPanel(null,true);else if(roaming)chooseView('overview');else if(document.body.classList.contains('reduced'))$('exitImmersion').click();return;}
