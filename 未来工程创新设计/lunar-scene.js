...
const roomModelTypes=roomTypes.map(type=>[5,3,0,2,5,7][type]);roomModelTypes[13]=1;roomModelTypes[19]=4;roomModelTypes[11]=6;
...
const roomAtlas=texture(2048,512,(ctx,w,h)=>{const cw=w/6,ch=h/4;roomTypes.forEach((type,id)=>{const x=(id%6)*cw,y=Math.floor(id/6)*ch,palette=roomPalette[type];ctx.fillStyle='#b9bdbb';ctx.fillRect(x,y,cw,ch);ctx.fillStyle=palette.color;ctx.fillRect(x+12,y+9,30,6);ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillStyle='#282b2b';ctx.font='700 74px Novecento Wide Bold, Lunaris Display, Lunaris Serif';ctx.fillText('R'+String(id+1).padStart(2,'0'),x+cw/2,y+63);ctx.fillStyle=palette.color;ctx.font='700 24px Novecento Wide Bold, Lunaris Display, Lunaris Serif';ctx.fillText(moduleProfiles()[roomModelTypes[id]].code,x+cw/2,y+108);});},null,true);
...
for(const sign of [-1,1]){identityFace(16,6,[-34,55,sign*(roomDepth/2-.08)],[0,sign<0?Math.PI:0,0]);identityFace(16,6,[sign*(roomWidth/2-.08),55,32],[0,sign*Math.PI/2,0]);}
