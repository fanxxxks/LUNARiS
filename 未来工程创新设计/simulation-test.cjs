...
assert.deepEqual([width,height,depth],[115,78,105],'room footprint grows in scene geometry while height stays fixed');
assert.equal(C.config.moduleScaleXZ,1.25);assert.equal(C.config.interiorWidth,102.5);assert.equal(C.config.interiorDepth,91.25);
assert.equal(width*C.config.metresPerUnit,11.5);assert.equal(depth*C.config.metresPerUnit,10.5);
const usableFloorArea=(C.config.interiorWidth*C.config.interiorDepth-(14*C.config.moduleScaleXZ)**2)*C.config.metresPerUnit**2;
assert.ok(Math.abs(usableFloorArea-90.46875)<1e-8,'rectangular floor allowance is 90.47 square metres after the 1.75 m square ladder opening, before rounded corners and fixtures');
...
for(const n of C.nodes){
 ...
 assert.deepEqual(C.slots[n.id],[n.col*125,34+n.level*92,n.row*117]);
...
assert.equal(C.segmentClear([0,34,0],[250,34,0],[[125,34,0]]),false);
assert.equal(C.segmentClear([0,34,0],[250,34,0],[[125,126,0]]),true);
assert.equal(C.segmentClear([0,34,0],[250,34,0],[[125,34,107.99]]),false,'new depth plus 3-unit clearance rejects a near miss');
assert.equal(C.segmentClear([0,34,0],[250,34,0],[[125,34,108]]),true,'new depth plus 3-unit clearance is accepted at the boundary');
assert.equal(C.segmentClear([0,34,0],[0,34,234],[[117.99,34,117]]),false,'new width plus 3-unit clearance rejects a near miss');
assert.equal(C.segmentClear([0,34,0],[0,34,234],[[118,34,117]]),true,'new width plus 3-unit clearance is accepted at the boundary');
...
const offsets={'+X':[57.5,26,0],'-X':[-57.5,26,0],'+Y':[30,76.5,-28.75],'-Y':[30,.3,-28.75],'+Z':[0,26,52.5],'-Z':[0,26,-52.5]};assert.deepEqual(C.portOffsets,offsets);
...
for(const layout of layouts)for(const unavailable of [[],[0],[1,2,7]]){
 ...
 for(let a=0;a<42;a++)if(layout[a]!==null&&!unavailable.includes(layout[a]))for(let b=a+1;b<42;b++)if(layout[b]!==null&&!unavailable.includes(layout[b])){
  const delta=C.slots[a].map((v,k)=>Math.abs(v-C.slots[b][k]));if(delta.filter(v=>v>1e-7).length===1&&delta.some((v,k)=>Math.abs(v-[125,92,117][k])<1e-7))expected.push(a+':'+b);
 }
...
close(C.walkRoute(C.initial,0,1).distance,12.5,'enlarged direct X route between vestibules');close(C.walkRoute(C.initial,0,3).distance,11.7,'enlarged direct Z route between vestibules');close(C.walkRoute(C.initial,0,9).distance,20.95,'vertical route includes enlarged horizontal vestibule access and unchanged internal ladders');
