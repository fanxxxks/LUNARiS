...
function connectionWrite(cache,batch,index,origin,quaternion,local,scale,stretch=[1,1,1]){
 const d=cache.matrix;d.position.fromArray(origin).add(cache.v.set(local[0]*stretch[0],local[1]*stretch[1],local[2]*stretch[2]).applyQuaternion(quaternion));d.quaternion.copy(quaternion);d.scale.set(scale[0]*stretch[0],scale[1]*stretch[1],scale[2]*stretch[2]);d.updateMatrix();batch.setMatrixAt(index,d.matrix);
 // Three r160 submits each updateRange separately. A contiguous dirty span
 // turns thousands of 64-byte driver calls into one upload per material.
 batch.userData.dirtyStart=Math.min(batch.userData.dirtyStart??Infinity,index);
 batch.userData.dirtyEnd=Math.max(batch.userData.dirtyEnd??-1,index);
}
function commitConnectionMatrices(batch){
 const {dirtyStart,dirtyEnd}=batch.userData;if(dirtyEnd===undefined)return;
 const attribute=batch.instanceMatrix;let start=dirtyStart*16,end=(dirtyEnd+1)*16;
 // Preserve updates not yet submitted (for example while a parent is hidden).
 ...
 delete batch.userData.dirtyStart;delete batch.userData.dirtyEnd;
}
function updateConnectivity(st,options={}){
 ...
 const hideRooms=options.hideRooms??(typeof cutaway==='boolean'&&cutaway),optionKey=[show,floor,section,hideRooms].join('|');
 // Orbiting changes only the camera. Reuse the immutable simulation snapshot
 // and its already-committed hatches instead of rebuilding 24 doorway records.
 if(st===s.lastState&&optionKey===s.optionKey)return {changed:false,connections:s.connections||0};
 ...
 const faces=connectionFaces(),batches=[s.leaf,s.inset,s.handle];
 ...
 if(doorChanged){for(const m of batches){m.count=C.config.roomCount*12;commitConnectionMatrices(m);}changed=true;}
 const movingLink=s.links.some(edge=>edge.roomA===room||edge.roomB===room);
 const linkKey=graphKey+'|'+(movingLink?room+'|'+progress.toFixed(5):'static')+'|'+show+'|'+floor+'|'+section;
 if(linkKey!==s.linkKey){
  ...
  s.connections=count;
  for(const [batch,n]of [[s.shell,wi],[s.ribs,ri],[s.glass,gi]]){batch.count=n;commitConnectionMatrices(batch);}changed=true;
 }
 if(changed)renderer.shadowMap.needsUpdate=true;
 return {changed,connections:s.connections||0};
}
function clearConnectivity(){const s=connectivityCache.value;if(!s)return;for(const m of [s.leaf,s.inset,s.handle,s.shell,s.ribs,s.glass])m.count=0;s.doorKeys=[];s.graphKey=s.linkKey=null;s.lastState=null;s.optionKey=null;s.connections=0;renderer.shadowMap.needsUpdate=true;}
